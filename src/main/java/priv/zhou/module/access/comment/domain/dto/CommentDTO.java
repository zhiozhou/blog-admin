package priv.zhou.module.access.comment.domain.dto;

import com.fasterxml.jackson.annotation.JsonFormat;
import com.fasterxml.jackson.annotation.JsonInclude;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;
import lombok.experimental.Accessors;
import priv.zhou.common.domain.dto.DTO;
import priv.zhou.module.access.comment.domain.po.CommentPO;
import priv.zhou.module.access.visitor.domain.dto.VisitorDTO;
import priv.zhou.module.blog.blog.domain.dto.BlogDTO;

import javax.validation.constraints.NotBlank;
import java.util.Date;


/**
 * 评论 数据传输模型
 *
 * @author zhou
 * @since 2020.06.25
 */
@Getter
@Setter
@NoArgsConstructor
@Accessors(chain = true)
@JsonInclude(JsonInclude.Include.NON_NULL)
public class CommentDTO extends DTO<CommentPO> {


    /**
     *
     */
    private Integer id;

    /**
     * 博客
     */
    private BlogDTO blog;

    /**
     * 主题id
     */
    private Integer topicId;

    /**
     * 回复的评论
     */
    private CommentDTO repliedComment;


    /**
     * 访客
     */
    private VisitorDTO fromVisitor;

    /**
     * 内容
     */
    @NotBlank(message = "内容不可为空")
    private String content;
    private String contentLike;

    /**
     * 状态
     */
    private Integer state;
    private String stateStr;

    /**
     * 创建时间
     */
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss", timezone = "GMT+8")
    private Date gmtCreate;

    /**
     * 修改时间
     */
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss", timezone = "GMT+8")
    private Date gmtModified;


    public CommentDTO(CommentPO commentPO) {
        super(commentPO);
        blog = ofPO(commentPO.getBlog(), BlogDTO::new);
        repliedComment = ofPO(commentPO.getRepliedComment(), CommentDTO::new);
        fromVisitor = ofPO(commentPO.getFromVisitor(), VisitorDTO::new);
    }

    @Override
    public CommentPO toPO() {
        return super.toPO()
                .setBlog(toPO(blog))
                .setRepliedComment(toPO(repliedComment))
                .setFromVisitor(toPO(fromVisitor));
    }
}
